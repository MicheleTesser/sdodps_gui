...
pub fn export_from_cached_values(
    database: &Database,
    values: &HashMap<(String, u16), (Value, std::time::Instant)>,
) -> ExportFile {
    let mut entries = Vec::new();

    for ((board_name, var_id), (value, _)) in values {
        let Some(board) = database.sdo_boards.iter().find(|item| item.name == *board_name) else {
            continue;
        };
